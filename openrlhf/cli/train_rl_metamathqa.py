...
def train(args):
    ...
    # prepare datasets
    prompts_data = prepare_metamathqa_dataset(
        dataset_path=args.dataset_path, 
        mode="train", 
        strategy=strategy, 
        max_count=args.max_samples
    )
    eval_data = prepare_metamathqa_dataset(
        dataset_path=args.dataset_path, 
        mode="eval_gsm8k", 
        strategy=strategy
    )
    prompts_dataset = MetaMathQASFTDataset(
        dataset=prompts_data, 
        tokenizer=tokenizer, 
        max_length=args.prompt_max_len, 
        strategy=strategy
    )
    eval_dataset = MetaMathQASFTDataset(
        dataset=eval_data, 
        tokenizer=tokenizer, 
        max_length=args.prompt_max_len, 
        strategy=strategy
    ...
    strategy.save_model(
        ...
        tokenizer, 
        os.path.join(args.save_path, "final")
    )
    strategy.save_model(
        critic, 
        tokenizer, 
        os.path.join(args.save_path, "final_critic")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_samples", type=int, default=None, help="Max number of prompts")
